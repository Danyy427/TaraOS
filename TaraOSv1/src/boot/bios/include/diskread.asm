...
;
; Reads n sector using non-extended BIOS functions
; Params: dl: drive number
;         cl: sector number 0-5, cylinder 6-7
;         ch: cylinder number
;         dh: head number
;         al: sector amount to read
;         es:bx: data buffer
; Output: ah = 0x11 on error
;         ah = 0x00 on sucess
;         al = 0x01 on success
...
readNSectorExtended:
    ...
    mov dword [b1], ecx ; Starting block ; Do Illegal stuff
    mov word [o1], si ; Buffer offset
    mov word [s1], ds ; Buffer segment
    mov word [i1], bx ; Amount of Sectors to read
    
    xor ax, ax
    mov ds, ax
    mov si, DAPACK1 ; address of data pack
    mov ah, 0x42
    int 0x13 ; Read sectors
...
DAPACK1:
    db 0x10 ; packet size
    db 0x00 ; reserved 
i1: dw 0x00 ; transfer size in sectors
o1: dw 0x0000 ; transfer buffer offset
s1: dw 0x0000 ; segment
b1: dd 0x00 ; starting block
    dd 0x00